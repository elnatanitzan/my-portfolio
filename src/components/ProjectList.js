import AlphaBetaGame from "../project-photos/Alpha-beta-Game.png";
import ApiExplorer from "../project-photos/Angular-Api-explorer.png";
import CookBook from "../project-photos/Cook-Book-Hooks-Api-Server.png";
import FindTheNumber from "../project-photos/Find-the-number-Game.png";
import MemoryGameJS from "../project-photos/Memory-Game-vanilla-JS.png";
import ReactMemoryGame from "../project-photos/React-Memory-Game.png";
import TodoAppReactFirebase from "../project-photos/TodoApp-React-Firebase.png";
import Todos3MethodsReact from "../project-photos/Todos-3-Methods-React.png";
import TooltipUI from "../project-photos/TooltipUI-mini-Library.png";
import SetButtonsSASS from "../project-photos/set-Buttons-use-SASS-@mixin.png";
import RenanaNitzanStore from "../project-photos/renana-nitzan-store.png";
import AngularTodos from "../project-photos/Angular-Todos.png";

export const Cards = [
        {
            src: CookBook,
            title: "CookBook",
            details: "An API Explorer I built in Angular, you can search and filter to find an API.",
            githubLink: "https://github.com/elnatanitzan/recipes-app",
            codepenLink: "",
            demoLink: "https://elnatanitzan.github.io/recipes-app/"
        },
        {
            src: AlphaBetaGame,
            title: "Alpha Beta Game",
            details: "Game Find the right character. Built in VanillaJS.",
            githubLink: "https://github.com/elnatanitzan/Alpha-beta--Game",
            codepenLink: "",
            demoLink: "https://elnatanitzan.github.io/Alpha-beta--Game/"
        },
        {
            src: ApiExplorer, 
            title: "Angular Api Explorer",
            details: "An API Explorer I built in Angular, you can search and filter to find an API.",
            githubLink: "https://github.com/elnatanitzan/API-explorer",
            codepenLink: "",
            demoLink: "https://elnatanitzan.github.io/API-explorer/"
        },
        {
            src: AngularTodos, 
            title: "Angular Todos",
            details: "ToDo app using Angular EventEmitter, Output & Input.",
            githubLink: "https://github.com/elnatanitzan/Todo-s-Angular",
            codepenLink: "",
            demoLink: "https://todos-angular-elnatanitzan.vercel.app/"
        },
        {
            src: FindTheNumber,
            title: "Find the number Game",
            details: "A game I built in VanillaJS to find the right number.",
            githubLink: "https://github.com/elnatanitzan/Game--Find-the-number.",
            codepenLink: "",
            demoLink: "https://elnatanitzan.github.io/Game--Find-the-number/"
        },
        {
            src: MemoryGameJS,
            title: "Memory Game VnillaJS",
            details: "A Memory game I built in VanillaJS.",
            githubLink: "https://github.com/elnatanitzan/Memory-Game--using-vanilla-JS",
            codepenLink: "",
            demoLink: "https://elnatanitzan.github.io/Memory-Game--using-vanilla-JS/"
        },
        {
            src: RenanaNitzanStore,
            title: "Renana Nitzan Online Store",
            details: "An online store I built and designed for my wife's business, on the WIX platform.",
            githubLink: "",
            codepenLink: "",
            demoLink: "https://www.renananitzan.com/"
        },
        {
            src: ReactMemoryGame,
            title: "React Memory Game",
            details: "A Memory game I built in React use Hooks",
            githubLink: "https://github.com/elnatanitzan/React-Memory-Game",
            codepenLink: "",
            demoLink: "https://elnatanitzan.github.io/React-Memory-Game/"
        },
        {
            src: TodoAppReactFirebase,
            title: "TodoApp React Firebase",
            details: "I built in React with MaterialUI and Hooks, in the connection of Firebase's cloud services and Authentication.",
            githubLink: "https://github.com/elnatanitzan/todos-react-firebase",
            codepenLink: "",
            demoLink: "https://elnatan-todos.web.app/"
        },
        {
            src: Todos3MethodsReact,
            title: "Todos app React 3 Methods",
            details: "Here i'm creating 3 versions: 1. using regular state in class components. 2. using Redux. 3. using Hooks in Functions Components.",
            githubLink: "https://github.com/elnatanitzan/Todos-App-using-React",
            codepenLink: "",
            demoLink: "https://elnatanitzan.github.io/Todos-App-using-React"
        },
        {
            src: TooltipUI,
            title: "TooltipUI mini Library",
            details: "Pure CSS/SCSS Tooltip Library. easy to use, clean design & No need for JS at all.",
            githubLink: "https://github.com/elnatanitzan/tooltip-ui",
            codepenLink: "https://codepen.io/elnatanitzan/pen/wvzZeRz",
            demoLink: "https://codepen.io/elnatanitzan/pen/wvzZeRz"
        },
        {
            src: SetButtonsSASS,
            title: "Set of buttons using SASS @mixin",
            details: "set beautiful Buttons I created use SCSS @mixin.",
            githubLink: "https://github.com/elnatanitzan/set-Buttons-SASS--mixin",
            codepenLink: "https://codepen.io/elnatanitzan/pen/PoGQBXp",
            demoLink: "https://elnatanitzan.github.io/set-Buttons-SASS--mixin/"
        }
]
